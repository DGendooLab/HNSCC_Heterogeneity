...
head(ann450k)
```
Reading in the Sample sheet which has parameters regarding the samples like Sample Number, sample subtype etc.
```{r}
# read in the sample sheet for the experiment
targets<- read.metharray.sheet("~/DATA/Sample Sheets", pattern="GB160121-KP Sample Sheet 450k_alltissues copy 2.csv") 
```
...
plotMDS(bVals, top=1000, gene.selection="common", 
        col=pal[factor(targets$Sample_HPV)], cex=1, labels = targets$Sample_Name)
legend("topright", legend=levels(factor(targets$Sample_HPV)), text.col=pal,
       cex=1, bg="white",)
```
...
```{r}
corr_mtx<-cor(bVals ,method = "spearman")
corr_mtx
library(viridis)
...
library(ggplot2)
t_bval<-t(bVals)
pca_results<-prcomp(t_bval,scale=TRUE)
pca_df<-as.data.frame(pca_results$x[,1:2])
pca_df$sample<-rownames(pca_df)
category_colors <- c("Core" = "red", "Plasma_DNA" = "blue", "Margin" = "green", "Normal"="pink")

ggplot(data=pca_df,aes(x=PC1, y=PC2, label=sample))+
  geom_point()+
  geom_text(hjust=0,nudge_x=0.1)+
  labs(x="Principal Component 1", y= "Principal Component 2")+scale_color_manual(values = category_colors)+ theme_minimal()
```
...
Tumour and non tumour, hpv and non HPV diagrame
```{r}
# read in the sample sheet for the experiment
targets_all_tum <- read.metharray.sheet("~/DATA/Sample Sheets", pattern="GB160121-KP Sample Sheet 450k_alltissuesTUMOUR.csv") 
targets_nonHPV_tum<- read.metharray.sheet("~/DATA/Sample Sheets", pattern="GB160121-KP Sample Sheet 450k_noHPVTumour.csv") 
targets_plasma<- read.metharray.sheet("~/DATA/Sample Sheets", pattern="GB160121-KP Sample Sheet 450k_plasma.csv") 
```

Read and load DNA methylation data from an Illumina methylation array experiment called rgSet. The function read.metharray.exp() takes the targets file as input and processes it to extract the necessary information and load the methylation data into rgSet.
```{r}
# read in the raw data from the IDAT files
rgSet_all_tum <- read.metharray.exp(targets=targets_all_tum)
rgSet_noHPV <- read.metharray.exp(targets=targets_nonHPV_tum)
rgSet_plasma <- read.metharray.exp(targets=targets_plasma)
```
Giving descriptive names to the samples
```{r}
# give the samples descriptive names
targets_all_tum$ID <- paste(targets_all_tum$Sample_Name,targets_all_tum$Sample_Group,sep=".")
sampleNames(rgSet_all_tum) <- targets_all_tum$ID
rgSet_all_tum

targets_nonHPV_tum$ID <- paste(targets_nonHPV_tum$Sample_Name,targets_nonHPV_tum$Sample_Group,sep=".")
sampleNames(rgSet_noHPV) <- targets_nonHPV_tum$ID
rgSet_noHPV

targets_plasma$ID <- paste(targets_plasma$Sample_Name,targets_plasma$Sample_Group,sep=".")
sampleNames(rgSet_plasma) <- targets_plasma$ID
rgSet_plasma
```
...
```{r}
# calculate the detection p-values
detP_all_tum <- detectionP(rgSet_all_tum)
detP_nonHPV_tum <- detectionP(rgSet_noHPV)
detP_plasma <- detectionP(rgSet_plasma)
```

Filtering out poor quality samples and keeping samples that have detection p <0.05
```{r}
# remove poor quality samples
keep_all_tum <- colMeans(detP_all_tum) < 0.05
rgSet_all_tum <- rgSet_all_tum[,keep_all_tum]

keep_nonHPV_tum <- colMeans(detP_nonHPV_tum) < 0.05
rgSet_noHPV <- rgSet_noHPV[,keep_nonHPV_tum]

keep_plasma <- colMeans(detP_plasma) < 0.05
rgSet_plasma <- rgSet_plasma[,keep_plasma]
```

Updating the smaple sheet to have only those samples that satisfy the treshold (p<0.05)
```{r}
# remove poor quality samples from targets data
targets_all_tum <- targets_all_tum[keep_all_tum,]
targets_nonHPV_tum<-targets_nonHPV_tum[keep_nonHPV_tum,]
targets_plasma<-targets_plasma[keep_plasma,]
```
Filter out detection p values table for only those that are present in the sample sheet (i.e only those that have satisfied the treshold of p<0.05)
```{r}
# remove poor quality samples from detection p-value table
detP_all_tum <- detP_all_tum[,keep_all_tum]
detP_nonHPV_tum<- detP_nonHPV_tum[,keep_nonHPV_tum]
detP_plasma<- detP_plasma[,keep_plasma]
```
...
```{r}
# normalize the data; this results in a GenomicRatioSet object
mSetSq_all_tum <- preprocessFunnorm(rgSet_all_tum) 
mSetSq_nonHPV_tum <- preprocessFunnorm(rgSet_noHPV) 
mSetSq_plasma<- preprocessFunnorm(rgSet_plasma) 
```

```{r}
# ensure probes are in the same order in the mSetSq and detP objects
detP_all_tum <- detP_all_tum[match(featureNames(mSetSq_all_tum),rownames(detP_all_tum)),] 
detP_nonHPV_tum <- detP_nonHPV_tum[match(featureNames(mSetSq_nonHPV_tum),rownames(detP_nonHPV_tum)),]
detP_plasma <- detP_plasma[match(featureNames(mSetSq_plasma),rownames(detP_plasma)),]
# remove any probes that have failed in one or more samples
keep_all_tum <- rowSums(detP_all_tum < 0.01) == ncol(mSetSq_all_tum) 
keep_nonHPV_tum <- rowSums(detP_nonHPV_tum < 0.01) == ncol(mSetSq_nonHPV_tum) 
keep_plasma <- rowSums(detP_plasma < 0.01) == ncol(mSetSq_plasma) 
```

```{r}
mSetSqFlt_all_tum <- mSetSq_all_tum[keep_all_tum,]
mSetSqFlt_nonHPV_tum <- mSetSq_nonHPV_tum[keep_nonHPV_tum,]
mSetSqFlt_plasma <- mSetSq_plasma[keep_plasma,]
```

```{r}
# if your data includes males and females, remove probes on the sex chromosomes
keep_all_tum <- !(featureNames(mSetSqFlt_all_tum) %in% ann450k$Name[ann450k$chr %in% 
                                                        c("chrX","chrY")])
mSetSqFlt_all_tum <- mSetSqFlt_all_tum[keep_all_tum,]
# remove probes with SNPs at CpG site
mSetSqFlt_all_tum <- dropLociWithSnps(mSetSqFlt_all_tum)


# if your data includes males and females, remove probes on the sex chromosomes
keep_nonHPV_tum <- !(featureNames(mSetSqFlt_nonHPV_tum) %in% ann450k$Name[ann450k$chr %in% 
                                                        c("chrX","chrY")])
mSetSqFlt_nonHPV_tum<- mSetSqFlt_nonHPV_tum[keep_nonHPV_tum,]
# remove probes with SNPs at CpG site
mSetSqFlt_nonHPV_tum <- dropLociWithSnps(mSetSqFlt_nonHPV_tum)


# if your data includes males and females, remove probes on the sex chromosomes
keep_plasma<- !(featureNames(mSetSqFlt_plasma) %in% ann450k$Name[ann450k$chr %in% 
                                                        c("chrX","chrY")])
mSetSqFlt_plasma <- mSetSqFlt_plasma[keep_plasma,]
# remove probes with SNPs at CpG site
mSetSqFlt_plasma <- dropLociWithSnps(mSetSqFlt_plasma)
```
...
# exclude cross reactive probes 
xReactiveProbes <- read.csv(file=paste(dataDirectory,"48639-non-specific-probes-Illumina450k.csv",sep="/"), stringsAsFactors=FALSE)
keep_all_tum <- !(featureNames(mSetSqFlt_all_tum) %in% xReactiveProbes$TargetID)
mSetSqFlt_all_tum <- mSetSqFlt_all_tum[keep_all_tum,] 

keep_nonHPV_tum<- !(featureNames(mSetSqFlt_nonHPV_tum) %in% xReactiveProbes$TargetID)
mSetSqFlt_nonHPV_tum <- mSetSqFlt_nonHPV_tum[keep_nonHPV_tum,] 

keep_plasma<- !(featureNames(mSetSqFlt_plasma) %in% xReactiveProbes$TargetID)
mSetSqFlt_plasma <- mSetSqFlt_plasma[keep_plasma,] 
```

```{r}
# calculate M-values for statistical analysis
mVals_all_tum <- getM(mSetSqFlt_all_tum)
mVals_nonHPV_tum <- getM(mSetSqFlt_nonHPV_tum)
mVals_plasma<- getM(mSetSqFlt_plasma)
```

```{r}
bVals_all_tum <- getBeta(mSetSqFlt_all_tum)
write.table(bVals_all_tum, file="All_Tumour_bval.csv", sep=",", row.names=TRUE)
bVals_nonHPV_tum <- getBeta(mSetSqFlt_nonHPV_tum)
bVals_plasma <- getBeta(mSetSqFlt_plasma)
```


Correlation matrix for beta values
```{r}
corr_mtx_all_tum<-cor(bVals_all_tum ,method = "spearman")
library(viridis)
heatmap_res_all_tum<-heatmaply(corr_mtx_all_tum, 
          colors = viridis(100),
          main = "Correlation Heatmap")

corr_mtx_nonHPV_tum<-cor(bVals_nonHPV_tum ,method = "spearman")
library(viridis)
heatmap_res_nonHPV_tum<-heatmaply(corr_mtx_nonHPV_tum, 
          colors = viridis(100),
          main = "Correlation Heatmap")

corr_mtx_plasma<-cor(bVals_plasma ,method = "spearman")
library(viridis)
heatmap_plasma<-heatmaply(corr_mtx_plasma, 
          colors = viridis(100),
          main = "Correlation Heatmap")

heatmap_res_all_tum
heatmap_res_nonHPV_tum
heatmap_plasma
```

```{r}

plotMDS(bVals_all_tum, top=1000, gene.selection="common", 
        col=pal[factor(targets_all_tum$Sample_HPV)], cex=1.5, labels = targets_all_tum$Sample_Name)
legend("topright", legend=c("HPV -ve","HPV +ve"), text.col=pal,
       cex=1.5, bg="white",)
#levels(factor(targets_all_tum$Sample_HPV))
plotMDS(bVals_plasma, top=1000, gene.selection="common", 
        col=pal[factor(targets_plasma$Sample_HPV)], cex=1.5, labels = targets_plasma$Sample_Name)
legend("topright", legend=c("HPV -ve","HPV +ve"), text.col=pal,
       cex=1.5, bg="white",)
#levels(factor(targets_plasma$Sample_HPV))
```
