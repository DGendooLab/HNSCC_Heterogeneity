...
targets_hpvana <- read.metharray.sheet("~/DATA/Sample Sheets", pattern="GB160121-KP Sample Sheet 450k_alltissuesTUMOUR.csv")
```
...
Updating the sample sheet to have only those samples that satisfy the threshold (p<0.05)
```{r}
# remove poor quality samples from targets data
targets_hpvana <- targets_hpvana[keep_hpvana,]
targets_hpvana[,1:5]
```
Filter out detection p values table for only those that are present in the sample sheet (i.e only those that have satisfied the threshold of p<0.05)
```{r}
...
```
```{r}
pdf("~/HPVNvsHPVP_DMRanalysis.pdf")
